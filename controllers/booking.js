  ...
  return Booking.find({ flightId: flightId })
    ...
    .then((bookings) => {
      if (bookings && bookings.length > 0) {
        ...
        res
          .status(200)
          .send({ message: "No bookings found for this flight", status: 200 });
      }
    })
    .catch((err) => res.status(500).send(err));
};
  ...
  return Booking.find({ userId: userId })
    ...
    .then((bookings) => {
      if (bookings && bookings.length > 0) {
        ...
        res
          .status(200)
          .send({ message: "No bookings found for this user", status: 200 });
      }
    })
    .catch((err) => res.status(500).send(err));
};
...
module.exports.createBookingWithPayment = async (req, res) => {
  ...
  try {
    ...
    const formattedSeats = selectedSeats.map((seat) => ({
      seatId:
        seat.dbId && mongoose.Types.ObjectId.isValid(seat.dbId)
          ? seat.dbId
          : undefined,
  ...
  return Booking.findByIdAndUpdate(
    ...
    .then((booking) => {
      if (!booking) {
        return res
          .status(404)
          .send({ message: "Booking not found", status: 404 });
      }
...
module.exports.getBookingsByAdmin = (req, res) => {
  return Booking.find()
    .then((bookings) => res.status(200).send({ bookings: bookings }))
